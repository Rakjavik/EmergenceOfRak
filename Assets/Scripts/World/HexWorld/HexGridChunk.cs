...
public class HexGridChunk : MonoBehaviour {
    ...
    public enum ChunkMaterial { GRASS, DIRT, SAND, SNOW }
    public enum ChunkElevationVariance { STEEP, HILLY, FLAT }
    public HexMesh terrain, rivers, roads, water, waterShore, estuaries;
    private ChunkMaterial chunkMaterial;
    private ChunkElevationVariance chunkElevation;
	HexCell[] cells;

	Canvas gridCanvas;

    public void SetCurrentOccupantForAllCells(BASE_SPECIES baseSpecies,
        string civName, bool defaults,World world)
    {
        for (int count = 0; count < cells.Length; count++)
        {
            if (cells[count].IsUnderwater) continue;
            Tribe tribe = new Tribe(1300,baseSpecies);
            cells[count].CurrentOccupants = tribe;
            world.addTribe(tribe,cells[count]);
        }
    }

    public void Initialize (ChunkElevationVariance elevation,ChunkMaterial material) {
        gridCanvas = GetComponentInChildren<Canvas>();
		cells = new HexCell[HexMetrics.chunkSizeX * HexMetrics.chunkSizeZ];
        ShowUI(true);
        this.chunkElevation = elevation;
	...
	public void AddCell (int index, HexCell cell) {
        ...
        cell.Initialize(chunkMaterial, chunkElevation);
	}
